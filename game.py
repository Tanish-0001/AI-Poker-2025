...
class PokerGame:
    ...
    def player_action(self, action: PlayerAction, amount: int = 0) -> bool:
        ...
        if action in [PlayerAction.BET, PlayerAction.RAISE]:
            ...
            # Reset has_played for all active players except the current one
            # This ensures everyone gets a chance to respond to the new bet/raise
            for i, p in enumerate(self.players):
                if i != self.active_player_index and p.status == PlayerStatus.ACTIVE:
                    self.has_played[i] = False
    ...
    def _display_game_state(self):
        ...
        print(f"Minimum raise: {self.min_raise}")
        ...
        for i, player in enumerate(self.players):
            ...
            active = "→ " if i == self.active_player_index and player.can_make_action() else "  "
            bet_info = f" (bet: {player.bet_amount})" if player.bet_amount > 0 else ""

            print(f"{active}{player.name} {position}: ${player.stack}{bet_info} {cards} {player.status.value}")
